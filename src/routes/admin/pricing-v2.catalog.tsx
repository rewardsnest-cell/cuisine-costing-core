...
function CatalogBootstrapPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Mapped-inventory gate removed — bootstrap ingests the full catalog */}
      {preflight.data && (
        <div className="text-xs text-muted-foreground">
          Mapping requirement removed — bootstrap will ingest the full available catalog.
          Currently <span className="font-mono">{preflight.data.mapped_count}</span> inventory
          item{preflight.data.mapped_count === 1 ? "" : "s"} mapped to a Kroger product (informational only).
        </div>
      )}
