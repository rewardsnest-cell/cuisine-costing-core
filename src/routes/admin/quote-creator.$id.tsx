...
import {
  ...
  type CqhDish, type CqhDocument, type CqhShoppingList, type CqhShoppingListItem,
...
import {
  ...
  Files, DollarSign, ClipboardList,
} from "lucide-react";

export const Route = createFileRoute("/admin/quote-creator/$id")({
  head: () => ({
    meta: [
      { title: "Quote Creator Workspace — Internal" },
      { name: "robots", content: "noindex, nofollow" },
    ],
  }),
  component: QuoteCreatorWorkspace,
});
...
function QuoteCreatorWorkspace() {
  const { id } = useParams({ from: "/admin/quote-creator/$id" });
    ...
    try {
      for (const f of Array.from(files)) {
        ...
        await addCqhDocument({ data: {
          event_id: id, filename: f.name, file_type: fileTypeLabel(f),
  ...
  const removeDoc = async (doc: CqhDocument) => {
    ...
    try {
      await removeCqhDocument({ data: { id: doc.id } });
  ...
  const extractDishes = async () => {
    setBusy(true);
    try {
      const res = await extractDishesFromDocs({ data: { event_id: id } });
  ...
  const addDish = async () => {
    ...
    try {
      await addCqhDish({ data: { event_id: id, name: newDish } });
      setNewDish("");
      load();
    } catch (e: any) { toast.error("Couldn't add dish", { description: e.message }); }
  };

  const toggleMain = async (d: CqhDish) => {
    await updateCqhDish({ data: { id: d.id, is_main: !d.is_main } });
    load();
  };

  const removeDish = async (d: CqhDish) => {
    if (!(await confirm({ title: "Delete dish?", description: d.name }))) return;
    await deleteCqhDish({ data: { id: d.id } });
    load();
  };
  ...
  const renameDish = async (d: CqhDish) => {
    ...
    await updateCqhDish({ data: { id: d.id, name: next } });
    load();
  };
  ...
  const generate = async () => {
    setBusy(true);
    try {
      const res = await generateShoppingList({ data: { event_id: id } });
  ...
  const approve = async () => {
    ...
    if (!(await confirm({ title: "Approve shopping list?", description: "Locks ingredients, quantities, and units. Pricing edits remain allowed.", destructive: false, confirmText: "Approve" }))) return;
    try {
      await approveShoppingList({ data: { shopping_list_id: data.currentList.id } });
      toast.success("Shopping list approved");
      load();
    } catch (e: any) { toast.error("Approval failed", { description: e.message }); }
  };
  ...
  const rebuild = async () => {
    ...
    setBusy(true);
    try {
      await generateShoppingList({ data: { event_id: id } });
  ...
  const mainCost = data?.items?.reduce((s, i) => {
    ...
    let mainShare = 0;
    for (const did of Object.keys(alloc)) {
      if (mainDishIds.has(did)) mainShare += Number(alloc[did]);
    }
  ...
  const createQuote = async () => {
    if (!data?.currentList || data.currentList.status !== "approved") {
      toast.error("Approve the shopping list first.");
      return;
    }
    setBusy(true);
    try {
      const { quote } = await createDraftQuoteFromCqh({ data: {
        event_id: id, shopping_list_id: data.currentList.id,
  ...
  if (loading || !data) return <div className="container mx-auto p-8"><LoadingState /></div>;
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <Link to="/admin/quote-creator" className="text-xs text-muted-foreground hover:underline">← All events</Link>
          <h1 className="font-display text-2xl font-bold mt-1 flex items-center gap-2">
            <ClipboardList className="w-5 h-5 text-primary" /> {ev.name}
          </h1>
          <p className="text-xs text-muted-foreground">
            {ev.event_date ?? "No date"} · {ev.guest_count ?? "?"} guests · revision {list?.revision_number ?? "—"}
          </p>
        </div>
        <ProgressBar status={ev.status} />
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
          ...
          {data.quotes.length > 0 && (
            <Card>
              ...
              <CardContent>
                <ul className="divide-y">
                  {data.quotes.map((q) => (
                    <QuoteRow key={q.id} q={q} onChanged={load} />
                  ))}
                </ul>
              </CardContent>
            </Card>
        ...
        <TabsContent value="shopping">
          {list ? (
            <ShoppingListEditor list={list} items={data.items} dishes={data.dishes} onChanged={load} isApproved={isApproved} />
          ) : (
              ...
              <ul className="divide-y">
                {data.documents.map((d) => (
                  <li key={d.id} className="py-2 text-sm flex justify-between">
                    ...
                    <span className="xs text-muted-foreground">{d.file_type} · {d.extracted_text ? `${d.extracted_text.length} chars` : "no text"}</span>
                  </li>
                ))}
              </ul>
            )}
